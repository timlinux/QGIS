...
#include "xpm/zoomin.xbm"
#include "xpm/cursorzoomin_mask.xpm"
#include "xpm/zoomout.xpm"
#include "xpm/pan.xpm"
#include "xpm/identify.xpm"

// XXX probably should s/.h/.xpm/
#include "xpm/pan.h"
#include "xpm/identify.h"
#include "xpm/selectcursor.h"
#include "xpm/zoom.h"
...
static const char * const qgisVersion = "0.0.12 - September 5, 2003";
...
QgisApp::QgisApp( QWidget * parent, 
                  const char * name, 
                  WFlags fl )
...
QgisApp::addLayer()
{
   mapCanvas->freeze(); // XXX what does this do?

   QStringList files = 
      QFileDialog::getOpenFileNames( "Shapefiles (*.shp);;All files (*.*)", 
                                     0, 
                                     this, 
                                     "open files dialog",
...
void
QgisApp::showMouseCoordinate(QgsPoint & p)
{
...
void
QgisApp::layerProperties(QListViewItem * lvi)
{
   QgsMapLayer * lyr;
...
void
QgisApp::rightClickLegendMenu(QListViewItem * lvi, const QPoint & pt, int)
{
   if (lvi)
      popMenu->exec(pt);
} // QgisApp::rightClickLegendMenu



QgisIface* 
QgisApp::getInterface()
{
...
// XXX ok, so what the hell is up with the magic number 99?
int
QgisApp::getInt()
{
   return 99;
...
void
QgisApp::actionPluginManager_activated()
{
...
void
QgisApp::loadPlugin( QString name, QString description, QString fullPath)
{
...
QgisApp::testMapLayerPlugins()
{
   ...
   {
      for ( unsigned int i = 0; i < mlpDir.count(); i++)
      {
         std::cout << "Getting information for plugin: " 
                   << mlpDir[i] << std::endl;
         ...
         if (loaded) 
         {
            std::cout << "Loaded test plugin library\n"
            ...
            if (cf) 
            {
               ...
               if (pl)
               {
                  ...
                  std::cout << "getInt returned " 
                            << pl->getInt() 
                            << " from map layer plugin\n";
                  // set up the gui
                  pl->initGui();
               }
               else
   ...
   if (false)
   {
      ...
      else
      {
         for( unsigned int i = 0; i < pluginDir.count(); i++)
         {
            ...
            QLibrary *myLib = 
               new QLibrary("../plugins/" + pluginDir[i]);//"/home/gsherman/development/qgis/plugins/" + pluginDir[i]);

            std::cout << "Library name is " << myLib->library() << std::endl;
            //QLibrary myLib("../plugins/" + pluginDir[i]);
            std::cout << "Attempting to load " 
                      << "../plugins/" + pluginDir[i] << std::endl;
            ...
            */
            ...
            if (loaded) 
            {
               ...
               std::cout << "Attempting to resolve the classFactory function\n";
               ...
               if (cf) 
               {
                  ...
                  std::cout << "Attempting to resolve the unload function\n";
...
} // QgisAp::socketConnected



void
QgisApp::socketConnectionClosed()
{
   QApplication::restoreOverrideCursor();
   // strip the header
